...
# 총평 호출 하나가 세 가지를 만든다 — 리포트 6줄, 흐름 서술, 섹터 카드 5장.
# 5회로는 부족해서 매크로 출처가 시장당 2~3건까지 떨어졌다(2026-08-07 실측).
OVERVIEW_SEARCH_MAX_USES = 12
...
9. source_url은 실제로 검색 결과에서 확인한 기사·공시 URL. 없으면 빈 문자열("").
   언론사 기사·거래소 공시·기업 IR 만 쓴다. 위키(나무위키·위키백과), 블로그,
   커뮤니티, 종목토론방은 출처로 쓰지 마라 — 그런 것뿐이면 빈 문자열로 두어라.
...
MACRO_MAX_ITEMS  = 9
SECTOR_MAX_ITEMS = 5
# 1면 흐름 본문 하드 캡. 1면이 비었던 건 캡이 낮아서가 아니라 모델이 캡을
# 안 채워서였다(실측 700자 내외 → 아래 175~220px 공백). 캡을 올리면 활자를
# 줄여야 하는 맞바꿈이라, 캡은 두고 프롬프트의 '하한'을 올렸다.
FLOW_MAX_CHARS   = 820
SECTOR_DRIVER_CLIP = 260   # 섹터 카드 설명 하드 캡 (프롬프트 지시는 200~240자)
...
def _overview_structured(client, prompt):
    ...
    instr = (
        ...
        "\n"
        "flow: 리포트 1면에 실릴 본문이자 뉴스 화면의 '시장의 흐름' 글이다.\n"
        "  lines 는 표 위 좁은 칸에 눌러 쓴 것이라 딱딱하다. 여기는 이어지는 글로 쓴다.\n"
        f"  분량 {FLOW_MAX_CHARS - 60}~{FLOW_MAX_CHARS}자. 이건 상한이 아니라 하한이 있는\n"
        "  지시다 — 할 말 다 했다고 600자에서 끊지 마라. 1면이 이 글로 채워지므로\n"
        "  짧으면 종이 아래가 빈 채로 인쇄된다. 분량이 남으면 인과의 다음 단계나\n"
        "  내일 확인할 지점을 더 써라.\n"
        ...
        "\n"
        f"macro_sources: 오늘 시장의 흐름을 설명해 주는 기사. 서로 다른 주제로 "
        f"6~{MACRO_MAX_ITEMS}건 담아라.\n"
        "  아래 sectors 에 건 기사는 화면에서 빠지므로, 그만큼 여유 있게 채워야 한다.\n"
        "  검색 횟수가 넉넉하니 주제가 모자라면 더 찾아봐라 — 3건에서 멈추지 마라.\n"
        "  ★ 여기는 '시장 전반' 칸이다. 개별 종목 기사는 넣지 마라.\n"
        "     넣을 것 : 지수·금리·환율·유가 등 시장 전체 / 업종 전반을 움직인 정책·규제 /\n"
        "               수급(외국인·기관 순매수) / 거시 지표.\n"
        "     넣지 말 것: 특정 회사 하나의 실적 발표, 목표주가 상향, 개별 공시 기사.\n"
        "               그건 종목 칸이 따로 담는다. 여기 넣으면 같은 기사가 두 번 실린다.\n"
        "     판단 기준: 그 기사가 종목 하나가 아니라 여러 종목·업종·지수를 설명하는가?\n"
        "  ★ 출처는 두 곳에서 나온다. 둘 다 쓸 수 있다:\n"
        "    (1) 매크로 원인을 확인하려고 네가 직접 검색해서 본 기사\n"
        "    (2) 위 [상승/하락 종목] 자료에 딸려 있는 출처 — 단, 위 기준을 통과할 때만.\n"
        "        업종 전반을 흔든 정책 발표라면 인용하고(다시 검색하지 마라),\n"
        "        그 회사 하나의 실적 기사면 인용하지 마라.\n"
        "  ★ 서로 내용이 겹치는 기사를 여러 건 넣지 마라. 한 주제당 가장 좋은 기사 1건만.\n"
        "     (같은 사건을 다룬 다른 매체 기사, 종합 시황 기사 중복 등은 하나로 줄여라)\n"
        "  ★ 출처는 언론사 기사·거래소 공시·기업 IR·통계기관 자료만 쓴다.\n"
        "     위키(나무위키·위키백과), 블로그, 커뮤니티, 종목토론방, 정체를 알 수 없는\n"
        "     사이트는 쓰지 마라. 쓸 만한 출처가 없으면 그 항목을 빼라 — 억지로\n"
        "     채우지 않아도 된다.\n"
        ...
        "\n"
        f"sectors: 오늘 크게 움직인 업종을 {SECTOR_MAX_ITEMS}개. 자료에 업종이 충분히\n"
        f"  있는데 2~3개만 내지 마라 — {SECTOR_MAX_ITEMS}개를 채워라.\n"
        ...
        "    리포트 1면의 카드 본문이라 짧으면 카드가 허전해진다. 3문장을 채워라.\n"
